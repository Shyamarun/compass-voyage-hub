...
import { Calendar, MapPin, User, DollarSign, Filter, Search, Eye, Edit, Trash2, Plus, CalendarCheck, Users, Ban, ArrowUp, ArrowDown, CheckCircle, Clock, XCircle } from 'lucide-react';
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative">
            ...
            <Input type="text" placeholder="Search name, package..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-10 w-64" />
          </div>
          
          {/* Filter Buttons */}
          {filterButtons.map(filter => <Button key={filter.key} variant={activeFilter === filter.key ? "default" : "outline"} size="sm" onClick={() => setActiveFilter(filter.key)} className="flex items-center gap-2">
              ...
              {bookings.map(booking => <tr key={booking.id} className="hover:bg-gray-50">
                  <td className="py-3 px-4">
                    <div className="flex items-center">
                      <img src={booking.customer.avatar} alt={booking.customer.name} className="w-8 h-8 rounded-full mr-3 object-cover" />
                      <div>
